...
public class ApplicationTest {
  ...
  //  @Test
    public void newOrderTest() {
        // Wait for maximum 5s until the first account gets inserted and processed
        NotifyBuilder notify = new NotifyBuilder(camelContext)
            .fromRoute("generate-account")
            .whenDone(2)
            .and()
            .fromRoute("process-account")
            .whenDone(1)
            .create();
        assertThat(notify.matches(10, TimeUnit.SECONDS)).isTrue();

        // Then call the REST API
        ResponseEntity<Account> orderResponse = restTemplate.getForEntity("/camel-rest-sql/books/account/1", Account.class);
        assertThat(orderResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
        Account account = orderResponse.getBody();
        assertThat(account.getId()).isEqualTo(1);
        /*assertThat(account.getAmount()).isBetween(1, 10);
        assertThat(account.getItem()).isIn("Camel", "ActiveMQ");
        assertThat(account.getDescription()).isIn("Camel in Action", "ActiveMQ in Action");
        assertThat(account.isProcessed()).isTrue();                 */
